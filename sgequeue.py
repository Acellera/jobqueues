...
class SGEQueue(SimQueue, ProtocolInterface):
    ...
    _defaults = {'queue': None, 'app': None, 'gpu_queue': None, 'cpu_queue': None,
    ...
    def __init__(self, _configapp=None):
        ...
        self._arg('outputstream', 'str', 'Output stream.', '$REQUEST.oJID[.TASKID]', val.String())
        self._arg('errorstream', 'str', 'Error stream.', '$REQUEST.eJID[.TASKID]', val.String())
        ...
        sgeconfig = _config['sge']
        profile = None
        if _configapp is not None:
            if sgeconfig is not None:
                if os.path.isfile(sgeconfig) and sgeconfig.endswith(('.yml', '.yaml')):
                    try:
                        with open(sgeconfig, 'r') as f:
                            profile = yaml.load(f)
                        logger.info('Loaded SGE configuration YAML file {}'.format(sgeconfig))
                    except:
                        logger.warning('Could not load YAML file {}'.format(sgeconfig))
                else:
                    logger.warning('{} does not exist or it is not a YAML file.'.format(sgeconfig))
                if profile:
                    try:
                        properties = profile[_configapp]
                    except:
                        raise RuntimeError('There is no profile in {} for configuration '
                                           'app {}'.format(sgeconfig, _configapp))
                    for p in properties:
                        setattr(self, p, properties[p])
                        logger.info('Setting {} to {}'.format(p, properties[p]))
            else:
                raise RuntimeError('No SGE configuration YAML file defined for the configapp')
        else:
            if sgeconfig is not None:
                logger.warning('SGE configuration YAML file defined without configuration app')
